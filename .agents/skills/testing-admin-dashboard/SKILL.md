...
- /create: six panels (funnel + medians, builds by rule, quality, relay and mirror, templates, budget) all from one `/api/admin/create?days=` read; with the mock down, every panel shows its own "Failed to load" line and the page still renders.
